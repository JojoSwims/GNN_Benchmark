...
# Every cached asset — bulk parquets and the country-code lookup tables —
# is fetched from this Drive folder on first use. The bulk parquets were
# produced by ``fetch_gdelt_data.py`` (~12h to regenerate from raw GDELT);
# the CSV lookup tables are static reference data.
_GDRIVE_FILES: dict[str, str] = {
    ...
    "fips_to_iso3.csv":   "1NCVRpqXJiaBCtUkST1EWGsITjyx4SkOf",
    "cow_to_iso3.csv":    "1kM1wOUAqyJ_TkWs3KjrervcfSIna9lpE",
    "iso3_centroids.csv": "1u1WpDpfh-FgCDb101-pYb15AhvA8hKH1",
}

_PARQUET_ASSETS = ("gdelt_all.parquet", "unga_votes.parquet", "manifest.json")
_CODES_ASSETS = ("fips_to_iso3.csv", "cow_to_iso3.csv", "iso3_centroids.csv")
...
def _drive_download_url(fid: str) -> str:
    # `confirm=t` bypasses Drive's virus-scan warning page that otherwise
    # wraps files >25 MB in HTML instead of the file bytes.
    return (
...
def _download_drive_file(name: str, fid: str, dest: Path) -> None:
    """Idempotent single-file Drive download with atomic rename."""
    if dest.exists():
        return
    print(f"  fetching {name} (id={fid})")
    tmp = dest.with_suffix(dest.suffix + ".part")
    with urllib.request.urlopen(_drive_download_url(fid), timeout=120) as resp, \
            open(tmp, "wb") as fh:
...
def _ensure_assets(cache: Path, names: tuple[str, ...]) -> None:
    """Download `names` from Drive into `cache` if they aren't already present."""
    cache.mkdir(parents=True, exist_ok=True)
    missing = [n for n in names if not (cache / n).exists()]
    if not missing:
        return
    print(f"[GDELT] Downloading {len(missing)} file(s) from Google Drive -> {cache}")
    for n in missing:
        _download_drive_file(n, _GDRIVE_FILES[n], cache / n)


def _ensure_codes() -> Path:
    """Ensure the country-code CSVs exist in the default cache; return its path.

    Lookup tables always live in the user-level cache (independent of any
    per-loader ``data_dir`` override) so the lazy module-level accessors
    don't need access to a loader instance.
    """
    cache = _default_cache_dir()
    _ensure_assets(cache, _CODES_ASSETS)
    return cache


@functools.cache
def _fips_to_iso3() -> dict[str, str]:
    p = _ensure_codes() / "fips_to_iso3.csv"
    return dict(pd.read_csv(p, dtype=str).itertuples(index=False, name=None))


@functools.cache
def _cow_to_iso3() -> dict[int, str]:
    p = _ensure_codes() / "cow_to_iso3.csv"
    return {
        int(cow): iso3
        for cow, iso3 in pd.read_csv(p, dtype={"cow": int, "iso3": str})
        .itertuples(index=False, name=None)
    }


@functools.cache
def _iso3_centroids() -> dict[str, tuple[float, float]]:
    p = _ensure_codes() / "iso3_centroids.csv"
    df = pd.read_csv(p)
    return {
...
@functools.cache
def _valid_iso3() -> set[str]:
    return set(_fips_to_iso3().values()) | set(_cow_to_iso3().values())
...
class GDELTProtestLoader(DatasetLoader):
    ...
    (downloaded from Google Drive on first use) and builds a
    diplomatic-similarity or geographic graph over the top-K most
    ...
    Attributes:
        ...
        data_dir: Optional directory of pre-fetched parquet files. When
            ...
            and auto-downloads everything from Google Drive on first use.
            Expected layout::

                <data_dir>/gdelt_all.parquet
    ...
    # Populated by ``download_and_convert``; ``info`` reflects whatever the
    # last download produced. Empty until the loader has been run.
    ...
    def download_and_convert(self) -> tuple[pd.DataFrame, pd.DataFrame]:
        ...
        if self.use_geographic_edges:
            ...
            edges_df = self._compute_unga_edges(cache, node_order)
    ...
    def _ensure_cache(cache: Path) -> None:
        """Populate ``cache`` from Google Drive if parquets are absent.

        Only the parquet/manifest assets are placed in the (possibly
        user-supplied) ``cache`` directory. Code-table CSVs are managed
        separately by ``_ensure_codes`` and always live in the default
        user-level cache.
        """
        _ensure_assets(cache, _PARQUET_ASSETS)
    ...
    def _aggregate_bins(events: pd.DataFrame) -> pd.DataFrame:
        ...
        events["action_iso3"] = events["action_country"].map(_fips_to_iso3())
    ...
    def _compute_geographic_edges(self, node_order: list[str]) -> pd.DataFrame:
        ...
        centroids = _iso3_centroids()
        rows: list[tuple[str, str, float]] = []
        coords = {n: centroids[n] for n in node_order if n in centroids}
    ...
    def _compute_unga_edges(
        self, cache: Path, node_order: list[str]
    ) -> pd.DataFrame:
        """UNGA voting similarity: cost = 1 - mean(agree) over recent window.

        Uses the precomputed dyadic agreement scores from the Voeten
        Dataverse (columns: ccode1, ccode2, agree, year). Prefers the
        cached parquet placed in ``cache`` by ``_ensure_cache``; falls back
        to a fresh Dataverse fetch only if the parquet is missing.
        """
        cached = cache / "unga_votes.parquet"
        if cached.exists():
            print(f"Loading cached UNGA agreement scores from {cached}")
            scores = pd.read_parquet(cached)
        else:
            with tempfile.TemporaryDirectory() as tmpdir:
                csv_path = self._download_unga_votes(Path(tmpdir))
        ...
        cow_map = _cow_to_iso3()
        scores["iso3_src"] = scores["ccode1"].map(cow_map)
        scores["iso3_dst"] = scores["ccode2"].map(cow_map)
