...
@dataclass
class LamaHCEDynamicLoader(DatasetLoader):
    """
    ...
    def info(self) -> DatasetInfo:
        ...
        return DatasetInfo(
            name=f"lamah_ce_dynamic_{self.resolution}",
            url=ZENODO_URL,
            frequency=freq,
    ...
    def download_and_convert(self) -> tuple[pd.DataFrame, pd.DataFrame | None]:
        ...
        print(f"[LamaHCE-dyn] {len(valid_ids)} gauges after gap filter "
    ...
    def _build_series(self, gauge_ids: list[int]) -> pd.DataFrame:
        ...
        if missing_qobs:
            print(f"[LamaHCE-dyn] {missing_qobs} qobs files not found -- skipped.")
        if missing_met:
            print(f"[LamaHCE-dyn] {missing_met} non-delineated gauges dropped.")
    ...
    def _build_edges(self, valid_ids: set[int]) -> pd.DataFrame | None:
        ...
        if not hierarchy_path.exists():
            print(f"[LamaHCE-dyn] {hierarchy_path} not found -- no edges.")
            return None
        ...
        if not rows:
            print("[LamaHCE-dyn] No valid river-network edges found.")
            return None

        print(f"[LamaHCE-dyn] {len(rows)} river-network edges built "
