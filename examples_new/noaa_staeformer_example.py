...
``adaptive_embedding_dim`` (drives ``model_dim`` = 24 + adaptive_emb).
Reg axis: ``dropout``.  Model-critical axis: ``num_layers`` (transformer
depth — the original STAEFormer paper grid tuned this).  ``num_heads=4``
is fixed and divides every model_dim this search produces (40, 64, 104).
"""
